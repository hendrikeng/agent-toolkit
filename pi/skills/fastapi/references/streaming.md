...
Check the pinned FastAPI version before choosing a pattern. Directly yielding from a path operation and `fastapi.sse` are latest-version features. On versions without them, return an explicit `StreamingResponse` or keep the project's established SSE integration; do not silently upgrade FastAPI or add a dependency.

## Stream JSON Lines

When the installed FastAPI version supports direct-yield path operations, declare the return type and use `yield` to return the data.
...
When the installed version provides `fastapi.sse`, use `response_class=EventSourceResponse` and `yield` items from the endpoint.
