...
Follow the repository's supported Python, FastAPI, and Pydantic versions and its existing conventions. Do not add a tool or library merely because this skill mentions it.

## Compatibility First

Inspect the pinned FastAPI version and available APIs before using examples below. Do not upgrade FastAPI just to use a newer pattern. In particular, `app.frontend()`, `router.frontend()`, `fastapi.sse`, and directly yielding from a path operation are latest-version features and may be unavailable in existing projects; use the compatible alternatives noted below.

## Quick Reference

* Serve frontend apps: when the installed version provides them, use `app.frontend()` or `router.frontend()`; otherwise keep the project's `StaticFiles` setup. See [Serve Frontend Apps](#serve-frontend-apps).
* Server-Sent Events (SSE): when `fastapi.sse` is available, use `response_class=EventSourceResponse` and `yield`; otherwise use the project's SSE integration or an explicit `StreamingResponse`. See [Streaming](#streaming-json-lines-sse-bytes) and [the streaming reference](references/streaming.md).
* JSON Lines and byte streaming: direct `yield` path operations are version-dependent; `StreamingResponse` is the compatible fallback. See [the streaming reference](references/streaming.md).
...
If the installed FastAPI version provides `app.frontend()`, use it to serve a built static frontend app, for example a directory generated by Vite, Astro, Angular, Svelte, Vue, or a similar tool.
...
When available, use `router.frontend()` when the frontend belongs to an `APIRouter`; normal router prefix behavior applies when the router is included.
...
`app.frontend()` and `router.frontend()` are low-priority routes: regular API routes are matched first, then frontend files and client-side routing fallbacks.

On older FastAPI versions, retain or use the project's existing `StaticFiles` mount instead:
...
Add a root mount after API routes, and preserve any existing SPA fallback behavior.
...
When the installed version provides `fastapi.sse`, stream Server-Sent Events with `response_class=EventSourceResponse` and `yield` items from the endpoint.
...
If `fastapi.sse` or direct-yield path operations are unavailable, preserve the project's existing SSE package or return an explicit `StreamingResponse` with `media_type="text/event-stream"`; do not silently upgrade FastAPI or add a package.
