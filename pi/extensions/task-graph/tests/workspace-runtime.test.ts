...
test("installer linkage loads only the new graph files and preserves old records", () => {
 ...
 for (const file of ["index.ts", "delivery.ts", "task-graph-core.ts", "workspaces.ts"]) assert.equal(readFileSync(join(agent, "extensions/task-graph", file), "utf8"), readFileSync(join(repo, "pi/extensions/task-graph", file), "utf8"))
