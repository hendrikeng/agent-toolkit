...
test("builds the interactive Orca planning prompt", () => {
	...
	assert.match(prompt, /keep the same thinking level for its one replacement attempt/)
	assert.match(prompt, /Do not increase thinking unless the user explicitly requests high/)
	...
	assert.match(chain, /Use medium thinking for every worker unless the user explicitly requests high/)
	assert.match(chain, /fresh worker at the same thinking level/)
	assert.match(chain, /Do not increase thinking unless the user explicitly requests high/)
	assert.doesNotMatch(chain, /risk-based thinking|fresh high-thinking worker/)
