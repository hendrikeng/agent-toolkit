...
import { acquireLease, assertGraphMode, assertGraphShell, assertNoLegacyGraph, digest, graphGit, graphLeaseState, LEGACY_GRAPH, literalPath, taskGraphPrompt } from "../task-graph-core.ts"
...
test("settled retirement rejects nonterminal, changed, missing, duplicate, or foreign orchestration evidence", () => {
 const cases: Array<[string, (f: ReturnType<typeof settledRetirementFixture>) => void, RegExp]> = [
  ...
  ["missing task", f => { f.tasks.length = 0 }, /ledger/],
  ["changed task", f => { f.tasks[0].spec += " changed" }, /ledger/],
  ["duplicate task", f => { f.tasks.push({ ...f.tasks[0], id: "task_duplicate" }) }, /ledger/],
  ["foreign task", f => { f.tasks[0].run_id = "run_foreign" }, /ledger/],
...
test("graph garbage inspection is mutation-free, state-specific, and credential-redacted", () => {
 ...
 assert.equal(report.inspectionOnly, true); assert.ok(report.records.some((item: any) => item.state === "eligible")); assert.equal(report.records.filter((item: any) => item.state === "pending-retirement").length, 2); assert.equal(report.records.find((item: any) => item.record.endsWith("symlink.json")).state, "uncertain"); assert.equal(report.records.find((item: any) => item.runId === "run_conflict" && item.location === "active").state, "uncertain"); assert.equal(report.records.find((item: any) => item.runId === "run_malformed_completed").state, "uncertain"); assert.ok(report.records.some((item: any) => item.state === "completed"), JSON.stringify(report)); assert.ok(report.records.some((item: any) => item.state === "retired")); assert.equal(report.records.find((item: any) => item.runId === "run_hashed_retired").state, "live-resource", JSON.stringify(report)); assert.ok(report.records.filter((item: any) => item.record.includes("symlink-")).every((item: any) => item.state === "uncertain")); assert.equal(report.records.find((item: any) => item.runId === "run_lost").state, "uncertain"); assert.ok(report.records.some((item: any) => item.state === "legacy")); assert.ok(report.records.some((item: any) => item.state === "uncertain"))
