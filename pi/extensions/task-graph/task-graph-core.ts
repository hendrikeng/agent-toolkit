...
Propose one to twelve tasks with literal owned paths, completion criteria, and exact setup and validation commands. Setup must prepare a clean lane before work: install required dependencies and generate required local artifacts. Do not use the main test or delivery action as setup. Validation must be repeatable, use native shell syntax instead of opaque wrappers, and leave tracked files unchanged; generate evidence before the final checkpoint. Include every required closeout lane in validation. Resolve the full unfinished plan dependency chain, with priority then Plan-ID ordering. Stop on missing, duplicate, draft, blocked or unapproved execution plans. Never invent external approval.
