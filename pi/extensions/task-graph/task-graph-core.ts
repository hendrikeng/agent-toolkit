...
export function validateTaskGraph(plan: TaskGraphPlan, root: string): void {
 ...
 if (plan.resources?.length && (plan.mode !== "execute" || !writingSources.size)) throw new Error("Graph resources require a writing task and integration workspace in execution mode.")
...
Propose one to twelve tasks with literal owned paths, completion criteria, and exact setup and validation commands. Setup must prepare a clean lane before work: install required dependencies and generate required local artifacts. Do not use the main test or delivery action as setup. For writing tasks, each setup and validation value must contain only a directly runnable shell command, never labels, steps, or prose. Validation must be repeatable, use native shell syntax instead of opaque wrappers, and leave tracked files unchanged; generate evidence before the final checkpoint. Include every required closeout lane in validation. Resolve the full unfinished plan dependency chain, with priority then Plan-ID ordering. Stop on missing or duplicate plans, dependency-ineligible or blocked plans, and approvals explicitly required before implementation. In execute mode, an explicit objective may promote only the next dependency-ready draft plan; draft status alone does not block admission. A pending delivered-code approval gates completion, not pre-implementation. Never invent external approval.
