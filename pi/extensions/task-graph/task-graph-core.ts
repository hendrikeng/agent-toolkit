...
After execute approval, run \`orca skills get orchestration\` and follow that version-matched guide. Confirm Orca is ready. Create or bind one Run, create the tasks with their dependencies, and start every ready independent worker before waiting. Use Orca for task state, dispatch, worker lifecycle, and messages. Do not recreate those features in Pi or in project files.
