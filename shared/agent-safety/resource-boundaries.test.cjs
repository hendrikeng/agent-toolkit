...
test('scanner scope verifies mounts, memory, expiry and shutdown without exposing host data', t => {
 const root = fs.mkdtempSync(path.join(process.env.AGENT_TOOLKIT_SCRATCH_ROOT, 'scanner-boundaries-'))
 for (const local of ['targets', 'signatures']) fs.mkdirSync(path.join(root, local))
 ...
 const prepare = () => prepareResources('scope', [declaration], state, () => {}, root, path.join(root, 'evidence'), runtime)
 prepare()
 ...
 item.Mounts[0].Source = path.join(root, 'signatures')
