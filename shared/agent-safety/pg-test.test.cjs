...
test('fixture administration is a separate fresh cluster, never superuser or an existing-database upgrade', async () => {
 ...
 assert.equal(admin.profile, 'maintenance-owner')
 assert.match(admin.database_url, /^postgresql:\/\/toolkit_maintenance_owner:[a-f0-9]{48}@127\.0\.0\.1:\d+\/toolkit_test$/)
 const queries = f.calls.filter(call => path.basename(call.file) === 'psql' && call.options.cwd === admin.path)
 assert.match(queries[0].options.input, /^CREATE ROLE toolkit_maintenance_owner LOGIN NOSUPERUSER NOCREATEDB CREATEROLE NOREPLICATION BYPASSRLS PASSWORD '[a-f0-9]{48}';\n$/)
