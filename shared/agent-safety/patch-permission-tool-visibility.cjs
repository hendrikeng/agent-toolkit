...
const PATCHES = {
 ...
 'access-intent/bash/command-enumeration.ts': [
  ...
  ['    // A command\'s text already contains any substitution; descend its subtree',
   '    const commandChildren = node.namedChildren.filter(child => child.type !== "variable_assignment" && !COMMAND_ENUM_SKIP.has(child.type));\n    const commandName = commandChildren.find(child => child.type === "command_name");\n    const rawName = commandName?.text ?? "";\n    const words = commandChildren.map(child => resolveNodeText(child.type === "command_name" ? child.child(0) ?? child : child));\n    words[0] = basename(words[0] ?? "").toLowerCase();\n    const dynamic = [rawName, words[0]].some(value => ["$", "\\\\", "*", "?", "[", "]", "{", "}", "~", "(", String.fromCharCode(96)].some(character => value.includes(character)));\n    out.push(makeUnit(words.join(" "), context, dynamic ? "opaque-payload" : classifyWrapperCommand(node)));\n    // A command\'s text already contains any substitution; descend its subtree'],
