...
then copy and immediately remove it:

```sh
handoff_path="/path/returned/by/mktemp"
pbcopy < "$handoff_path"
copy_status=$?
rm -f -- "$handoff_path"
exit "$copy_status"
...
Do not use a fixed `/tmp` filename. Avoid inline shell quoting for prompts
containing backticks, `$`, quotes, or user text. If copying fails, still remove
the temporary file before returning.
