...
Use the repository's risk gates to decide when to run a review. Ordinary edits
and passing tests do not trigger a review. Honor `reviews:off` and explicit
user instructions. Never install an AI review as a Git hook.

Keep accepted fixes inside the original task. Report adjacent problems as
follow-ups. If a fix requires a new contract or owner boundary, stop and ask.
On release work, fix only release blockers. After two unsuccessful fix cycles,
...
`AUTOREVIEW_MAX_PRIORITY` to select a different threshold. Do not add unrelated redesign goals or prescribe file counts, reading
